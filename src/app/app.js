...
process.on('uncaughtException', function(e) {
    log('uncauthg Exception happened...');
    log(e);
});
...
updater.on("download", function(version){
    log("OH YEAH! going to download version "+version)
})
updater.on("installed", function(){
    log("SUCCCESSFULLY installed new version, please restart")
})
...
var statusMessage = function(unchoked, wires, swarm) {
  document.getElementById('box-message').innerHTML = 
      ...
      + wires.length
      + '</br> Speed: '
      + bytes(swarm.downloadSpeed())
      + '/s</br>  Downloaded: '
      + bytes(swarm.downloaded);
};
...
/*var xmlRokuServer = function() {
  ...
  var server = http.createServer(function(req, res) {
    log('valor de global_href:', global_href);
    mu.clearCache();
  ...
  try {
    ...
    log('Coulnd\'t start roku App service.');
  }
};*/
...
doc.ondrop = function(event) {
    ...
    if (new_torrent.toLowerCase().substring(new_torrent.length-7,new_torrent.length).indexOf('torrent')>-1){
      ...
      if(new_torrent.toLowerCase().substring(new_torrent.length-3,new_torrent.length).indexOf('mp4')>-1
        ...
        log('going to check about: ' + new_torrent);
        if ( new_torrent.toLowerCase().substring(new_torrent.length-3,new_torrent.length).indexOf('srt') > -1
          || new_torrent.toLowerCase().substring(new_torrent.length-3,new_torrent.length).indexOf('vtt') > -1
            ...
            log('dirname and basename:');
            log(dirname);
            log(basename);
            log(new_torrent);

            if (new_torrent.toLowerCase().substring(new_torrent.length-3,new_torrent.length).indexOf('srt')>-1){
                log("converting srt and then creating server for: "+new_torrent);
                srt2vtt2(new_torrent, function(err, data) {
                    try {
                        subtitles_server.start(data, function(){
                ...
                log('creating server for: ' + new_torrent);
                subtitles_server.start({vtt: new_torrent, encoding: 'utf8'}, function(){
                    log('server restarted.');
                });
            }
        ...
        if(subtitlesDropped == false) {
            port++;
            log('creating new CORS server...');
            app.use(serveStatic(dirname)).listen(port);
            scfs.start(new_torrent, function(){log(">> Simple CORS server restarted.")})
        }

        if (torrenting==true) {
            log('global_href: ' + global_href);
            playInDevices(global_href, global_href);
        } else {
            log('playing not torrent');
            ...
            if (subtitlesDropped == true) {
                ...
                log('New resource with subtitles: ' + JSON.stringify(resource));
            }
...
function rewind30(n) {
    self.devices[n].seek(-30, function(time) {
      log('Rewinded 30secs!'+status);
      self.devices[n].togglePlayIcon('PAUSED');
    });
}

function togglePlay(n) {
      log(self.devices[n].streaming);
      log(self.devices[n].playing);
          ...
          if(self.devices[n].stopped == true){
            ...
            if(self.devices[n].loadingPlayer != true){
                ...
                self.devices[n].play(this.playingResource,0,function(err, status){
                    log(status)
                    log('telling to play from start again')
                    if(devices[n].togglePlayIcon){
                      log("Toggling play icon")
...
function toggleDevice(n) {
    ...
    log('toggling device ' + n);
    log(JSON.stringify(document.getElementById('off'+n).classList));
...
var gotTorrent = function(this_torrent) {
  ...
  engine.server.on('listening', function() {
    log('Streaming server is listening');
    ...
    log("("+bytes(filelength)+") "+filename.substring(0, 13)+"...");
